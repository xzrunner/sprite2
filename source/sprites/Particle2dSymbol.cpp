...
RenderReturn Particle2dSymbol::DrawTree(const RenderParams& rp, const SprConstPtr& spr) const
{
#ifndef S2_DISABLE_STATISTICS
...
//	StatSymDraw::DrawCostCP cp(STAT_SYM_PARTICLE2D);
	...
	*rp_child = rp;
	...
	auto& p2d_spr = S2_VI_PTR_DOWN_CAST<const Particle2dSprite>(spr);
...
RenderReturn Particle2dSymbol::DrawNode(cooking::DisplayList* dlist, const RenderParams& rp, const SprConstPtr& spr, ft::FTList& ft, int pos) const
{
	if (!spr) {
		return RENDER_NO_DATA;
	}
...
sm::rect Particle2dSymbol::GetBoundingImpl(const SprConstPtr& spr, const ActorConstPtr& actor, bool cache) const
{
